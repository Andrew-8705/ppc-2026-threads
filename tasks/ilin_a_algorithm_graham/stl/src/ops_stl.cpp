...
IlinAGrahamSTL::IlinAGrahamSTL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool IlinAGrahamSTL::ValidationImpl() {
  return !GetInput().points.empty();
}

bool IlinAGrahamSTL::PreProcessingImpl() {
  points_ = GetInput().points;
  hull_.clear();
  return true;
}

bool IlinAGrahamSTL::RunImpl() {
  if (points_.size() < 3) {
    hull_ = points_;
    return true;
  }
...
bool IlinAGrahamSTL::PostProcessingImpl() {
  GetOutput() = hull_;
  return true;
}
