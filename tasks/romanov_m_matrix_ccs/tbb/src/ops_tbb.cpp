#include "romanov_m_matrix_ccs/tbb/include/ops_tbb.hpp"
...
RomanovMMatrixCCSTBB::RomanovMMatrixCCSTBB(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool RomanovMMatrixCCSTBB::ValidationImpl() {
  return GetInput().first.cols_num == GetInput().second.rows_num;
}

bool RomanovMMatrixCCSTBB::PreProcessingImpl() {
  return true;
}

void RomanovMMatrixCCSTBB::MultiplyColumn(size_t col_index, const MatrixCCS &a, const MatrixCCS &b,
...
bool RomanovMMatrixCCSTBB::RunImpl() {
...
bool RomanovMMatrixCCSTBB::PostProcessingImpl() {
  return true;
}
