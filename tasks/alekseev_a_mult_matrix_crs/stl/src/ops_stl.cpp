...
AlekseevAMultMatrixCRSSTL::AlekseevAMultMatrixCRSSTL(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool AlekseevAMultMatrixCRSSTL::ValidationImpl() {
  const auto &a = std::get<0>(GetInput());
  const auto &b = std::get<1>(GetInput());
  return a.cols == b.rows && !a.row_ptr.empty() && !b.row_ptr.empty();
}

bool AlekseevAMultMatrixCRSSTL::PreProcessingImpl() {
  GetOutput() = {};
  return true;
}

void AlekseevAMultMatrixCRSSTL::ProcessRow(std::size_t i, const CRSMatrix &a, const CRSMatrix &b,
...
bool AlekseevAMultMatrixCRSSTL::RunImpl() {
  const auto &a = std::get<0>(GetInput());
  const auto &b = std::get<1>(GetInput());
  auto &c = GetOutput();
...
bool AlekseevAMultMatrixCRSSTL::PostProcessingImpl() {
  return true;
}
