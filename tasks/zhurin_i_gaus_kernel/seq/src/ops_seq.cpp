...
ZhurinIGausKernelSEQ::ZhurinIGausKernelSEQ(const InType &in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
  GetOutput() = OutType{};
}

bool ZhurinIGausKernelSEQ::ValidationImpl() {
...
bool ZhurinIGausKernelSEQ::PreProcessingImpl() {
  const auto &in = GetInput();
  width_ = std::get<0>(in);
...
bool ZhurinIGausKernelSEQ::RunImpl() {
...
bool ZhurinIGausKernelSEQ::PostProcessingImpl() {
  GetOutput() = result_;
