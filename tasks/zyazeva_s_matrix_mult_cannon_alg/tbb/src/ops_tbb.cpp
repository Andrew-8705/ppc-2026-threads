...
inline size_t BlockOffset(size_t row,
                          size_t col,
                          size_t grid_size,
                          size_t block_area) {
...
ZyazevaSMatrixMultCannonAlgTBB::ZyazevaSMatrixMultCannonAlgTBB(
    const InType& in) {
...
void ZyazevaSMatrixMultCannonAlgTBB::MultiplyBlocks(
    const double* a,
    const double* b,
    double* c,
    int block_size) {
...
bool ZyazevaSMatrixMultCannonAlgTBB::RunImpl() {
  ...
  const int sqrt_threads =
      static_cast<int>(std::sqrt(max_threads));
  ...
  tbb::parallel_for(
      ...
      [&](size_t block_id) {
        ...
        const size_t a_offset =
            BlockOffset(bi, bj, gs, block_area);

        for (size_t i = 0; i < bs; ++i) {
          const size_t global_i = bi * bs + i;

          const size_t src_offset =
              global_i * static_cast<size_t>(sz) + bj * bs;
          ...
          std::copy_n(
              m1.data() + src_offset,
              bs,
              blocks_a.data() + local_offset);

          std::copy_n(
              m2.data() + src_offset,
              bs,
              blocks_b.data() + local_offset);
        }
      });
  ...
  for (size_t i = 0; i < gs; ++i) {
    for (size_t j = 0; j < gs; ++j) {
      map_a[BlockIndex(i, j, gs)] =
          BlockIndex(i, (j + i) % gs, gs);

      map_b[BlockIndex(i, j, gs)] =
          BlockIndex((i + j) % gs, j, gs);
    }
  }
  ...
  for (size_t step = 0; step < gs; ++step) {
    ...
    tbb::parallel_for(
        ...
        [&](size_t idx) {
          ...
          const double* a_ptr =
              blocks_a.data() + a_idx * block_area;

          const double* b_ptr =
              blocks_b.data() + b_idx * block_area;

          double* c_ptr =
              blocks_c.data() + idx * block_area;
    ...
    if (step + 1 < gs) {
      ...
      for (size_t i = 0; i < gs; ++i) {
        for (size_t j = 0; j < gs; ++j) {
          next_a[BlockIndex(i, j, gs)] =
              map_a[BlockIndex(i, (j + 1) % gs, gs)];

          next_b[BlockIndex(i, j, gs)] =
              map_b[BlockIndex((i + 1) % gs, j, gs)];
        }
      }
      ...
      [&](size_t block_id) {
        ...
        for (size_t i = 0; i < bs; ++i) {
          const size_t dst_row =
              (bi * bs + i) * static_cast<size_t>(sz);

          const size_t src_row =
              block_offset + i * bs;

          std::copy_n(
              blocks_c.data() + src_row,
              bs,
              result.data() + dst_row + bj * bs);
        }
      });
