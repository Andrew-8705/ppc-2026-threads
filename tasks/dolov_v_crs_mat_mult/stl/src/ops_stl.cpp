...
bool DolovVCrsMatMultStl::ValidationImpl() {
...
double DolovVCrsMatMultStl::DotProduct(const SparseMatrix &matrix_a, int row_a, const SparseMatrix &matrix_b_t,
                                       int row_b) {
  double sum = 0.0;
...
bool DolovVCrsMatMultStl::RunImpl() {
  ...
  auto worker = [&](int start_row, int end_row) {
    for (int i = start_row; i < end_row; ++i) {
      ...
      for (int j = 0; j < matrix_b_t.num_rows; ++j) {
        double sum = DolovVCrsMatMultStl::DotProduct(matrix_a, i, matrix_b_t, j);
