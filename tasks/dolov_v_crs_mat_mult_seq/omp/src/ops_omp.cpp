...
bool DolovVCrsMatMultOmp::RunImpl() {
  ...
  int num_rows_a = matrix_a.num_rows;
  int num_rows_b_t = matrix_b_t.num_rows;

  std::vector<std::vector<double>> thread_vals(num_rows_a);
  std::vector<std::vector<int>> thread_cols(num_rows_a);

#pragma omp parallel for default(none) \
    shared(matrix_a, matrix_b_t, thread_vals, thread_cols, num_rows_a, num_rows_b_t) schedule(dynamic)
  for (int i = 0; i < num_rows_a; ++i) {
    for (int j = 0; j < num_rows_b_t; ++j) {
      double sum = DotProduct(matrix_a, i, matrix_b_t, j);

      if (std::abs(sum) > 1e-15) {
        thread_vals[i].push_back(sum);
        thread_cols[i].push_back(j);
      }
    }
  }

  for (int i = 0; i < num_rows_a; ++i) {
    result.values.insert(result.values.end(), thread_vals[i].begin(), thread_vals[i].end());
    result.col_indices.insert(result.col_indices.end(), thread_cols[i].begin(), thread_cols[i].end());
