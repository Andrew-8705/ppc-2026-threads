...
// Функция для параллельного поиска минимума
std::size_t FindLowestPointParallel(PointSet &points) {
  const int total_points = static_cast<int>(points.size());

  // Создаем локальную копию для OpenMP
  PointSet &local_points = points;
  int lowest_idx = 0;
...
#pragma omp critical
    {
      if (local_points[local_lowest].coord_y < local_points[lowest_idx].coord_y ||
          (std::abs(local_points[local_lowest].coord_y - local_points[lowest_idx].coord_y) < kTolerance &&
           local_points[local_lowest].coord_x < local_points[lowest_idx].coord_x)) {
...
void ParallelMergeSort(PointSet &points, int left, int right, const Point2D &origin) {
  if (left >= right) {
    return;
  }
...
// Функция для запуска параллельной сортировки
...
bool PetersonGrahamScannerOMP::RunImpl() {
  ...
  // Алгоритм Грэхема (ПОСЛЕДОВАТЕЛЬНЫЙ - так и должно быть!)
